...
class RoundOrchestrator:
    ...
    async def run_round(self, miner_submissions: dict[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        # 3. Generate held-out ground truth
        held_out_data = self._generate_ground_truth(test_case, test_period)
    ...
    def _generate_ground_truth(self, test_case: dict[str, Any], test_period: tuple[int, int]) -> dict[str, list[float]]:
        """Generate ground truth data for the held-out period.

        In local mode (no BOPTEST), runs the RC model with default parameters.
        In production, this would use BOPTEST emulator data.
