...
- `CALIBRATE.md`: tutorial on building energy calibration for newcomers
...
```
raw_scores = [composite_1, composite_2, ..., composite_n]
if sum(raw_scores) == 0:
    weights = [1/n for _ in raw_scores]  # Uniform if all infeasible
else:
    weights = [score / sum(raw_scores) for score in raw_scores]
```

Normalized to sum to 1.0 for Yuma Consensus weight setting. The zero-sum guard handles the rare case where all submissions are infeasible in a single round.
...
- Produces outputs outside physically reasonable bounds
- Has CVRMSE > 1.0 (worse than 100% error)
- Exceeds the verification timeout of 5 minutes (see Section 5.6)

The submission receives a score of 0.0 with a machine-readable reason code (SIMULATION_CRASHED, INVALID_PARAMS, ACCURACY_FLOOR, SIMULATION_TIMEOUT).
...
### 4.11 CVRMSE Normalization Note

The current CVRMSE normalization is linear: the difference between 0.05 and 0.10 (both excellent) is rewarded the same as the difference between 0.20 and 0.25 (both mediocre). This may under-reward marginal improvements at the top end where calibration is hardest. Non-linear mappings (e.g., exponential or sigmoid) that reward top-end precision more heavily are flagged as a testnet calibration item.
...
| Submitting default parameters | Scores 0.0 or near-zero on held-out data. No benefit. | Very high |
...
|-----------|-------|-----------|
| tempo | 360 (72 min) | Standard. One calibration round per tempo. |
...
| R9 | Scoring edge cases | M | H | Hardened numerics, adversarial testing, local eval harness |
