...
## 2. Phase 0: Foundation (Weeks 1 to 2)

**Goal:** Repository scaffolding, toolchain, and first proof-of-concept simulation run.

### Deliverables

- [ ] Initialize `zhen/` repository with directory structure from Architecture Document Section 10
- [ ] `pyproject.toml` with uv, ruff, mypy, pytest configuration
- [ ] `CLAUDE.md` routing table (Architecture Document Section 11.1)
- [ ] GitHub Actions CI: lint, type check, unit tests
- [ ] First RC network model: implement `RCNetworkBackend` from Architecture Document Section 4.2
- [ ] First test case `config.json` for `bestest_hydronic_heat_pump`
- [ ] Verify RC model runs locally: input parameters, output zone temperature + heating energy
- [ ] Physics sanity test: increasing insulation R-value reduces heating energy (automated test)

### Exit Criteria

RC network model runs, produces physically plausible outputs, and passes determinism test (same params = same outputs across 10 runs).

---

## 3. Phase 1: Scoring Engine (Weeks 3 to 4)

**Goal:** Complete scoring pipeline that takes predicted/measured time-series and produces composite scores.

### Deliverables

- [ ] Implement `compute_cvrmse`, `compute_nmbe`, `compute_r_squared` (definitions in Mechanism Design Section 4.2, implementations in Architecture Section 2.6)
- [ ] Implement `safe_clamp` normalization
- [ ] Implement `ScoringEngine.compute()` with all four components (CVRMSE 50%, NMBE 25%, R-squared 15%, convergence 10%)
- [ ] Implement weight-setting normalization with zero-sum guard
- [ ] Implement `EMATracker` from Architecture Document Section 2.8
- [ ] Implement score breakdown JSON generation (matching Mechanism Design Section 3.2 example)
- [ ] Verify score breakdown math: composite = 0.694 for the worked example values
- [ ] Unit tests: every edge case (zero submissions, all infeasible, single miner, all tied, NaN/Inf inputs)
- [ ] Unit tests: metric implementations with known inputs and hand-computed expected outputs

### Exit Criteria

Scoring engine passes all unit tests including adversarial inputs. Score breakdown JSON matches the worked example in the mechanism design document.

---

## 4. Phase 2: ZhenSimulator and Test Case Infrastructure (Weeks 5 to 7)

**Goal:** Complete simulation wrapper, first BOPTEST integration, and model registry.

### Deliverables

- [ ] Implement `ZhenSimulator` unified interface from Architecture Document Section 4.1
- [ ] Implement `_load_config` with `Path.home()` (not tilde)
- [ ] Implement weather data loading from local test case directory
- [ ] Create first complete test case: `bestest_hydronic_heat_pump`
  - [ ] RC network simplified model
  - [ ] `config.json` with parameter bounds, `scoring_outputs` list, defaults (Mechanism Design Section 4.6)
  - [ ] Pre-installed weather data (Brussels TMY)
  - [ ] README.md documentation
- [ ] BOPTEST integration: deploy `bestest_hydronic_heat_pump` emulator locally via Docker
- [ ] Implement `BOPTESTClient` REST API wrapper from Architecture Document Section 4.3
- [ ] Verify: ZhenSimulator(params) produces outputs that correlate with BOPTEST emulator outputs
- [ ] Implement `manifest.json` schema and loader
- [ ] Create Dockerfile for simplified model distribution
- [ ] Create Dockerfile for emulator distribution
- [ ] Determinism test: run simplified model 10x with same params, verify bit-identical outputs

### Exit Criteria

Full test case pipeline works: BOPTEST emulator generates ground truth, ZhenSimulator produces predictions, predictions and ground truth are in the same format for scoring.
...
- [ ] Accumulate testnet TAO (taoswap.org/testnet-faucet, 50 TAO per coldkey, ~275 to 300 total)
- [ ] Provision VPS for validator (4+ cores, 8+ GB RAM, Docker)
- [ ] Create subnet on testnet: `btcli subnet create --network test`
- [ ] Register owner validator + reference miner
...
- [ ] Add 2 more test cases (target: 3 total for testnet)
- [ ] Implement and deploy public dashboard
- [ ] Implement and deploy local eval harness (`zhen-eval`)
- [ ] Write MINE.md (30-minute cold start guide)
- [ ] Write SCORING.md (full formula + worked examples)
- [ ] Write RULES.md (explicit constraints)
- [ ] Write CALIBRATE.md (building energy calibration tutorial)
- [ ] Write VALIDATE.md (validator setup guide)
- [ ] Publish llms.txt
...
| ASHRAE compliance | Top miner achieves CVRMSE < 15% monthly on at least 2 test cases (relaxed from 4 in Mechanism Design Section 10.4 due to initial testnet launching with 3 test cases; target increases as library grows) |
...
| Test case diversity | 3+ test cases in rotation |
