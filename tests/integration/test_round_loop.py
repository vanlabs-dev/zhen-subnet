...
@pytest.mark.asyncio
async def test_full_round_local(tmp_path: Path) -> None:
    ...
    try:
        result = await _run_round(orchestrator, scoring_engine, scoring_db, verification_engine, submissions, "round-0")
        ...
        # Rows landed in the DB end-to-end
...
@pytest.mark.asyncio
async def test_two_rounds_ema(tmp_path: Path) -> None:
    """EMA smoothing over the windowed DB keeps weights normalized across rounds."""
    orchestrator = RoundOrchestrator(manifest_path=MANIFEST_PATH)
    scoring_engine = ScoringEngine()
    scoring_db = ScoringDB(db_path=tmp_path / "scoring.db")
    verification_engine = VerificationEngine()
    ...
    try:
        result1 = await _run_round(
            orchestrator, scoring_engine, scoring_db, verification_engine, submissions, "round-0"
        )
        weights_after_r1 = result1["weights"].copy()

        result2 = await _run_round(
            orchestrator, scoring_engine, scoring_db, verification_engine, submissions, "round-1"
        )
        ...
        # Both rounds persisted, so the windowed query should see 2 rounds
        assert len({r.round_id for r in result2["window_rows"]}) == 2
    finally:
        scoring_db.close()


@pytest.mark.asyncio
async def test_breakdown_format(tmp_path: Path) -> None:
